...
- [2. 環境構築](#2-環境構築)
  ...
  - [2.7. UbuntuにWireshark 3.6.11をインストール](#27-ubuntuにwireshark-3611をインストール)
    ...
    - [2.7.4. sudo権限がないユーザー向けの設定](#274-sudo権限がないユーザー向けの設定)
    - [2.7.5. SOME/IPパケットキャプチャの有効化](#275-someipパケットキャプチャの有効化)
...
#### 2.7.4. sudo権限がないユーザー向けの設定

次のコマンドを実行し現れる画面でYesを選択する。

```bash
sudo dpkg-reconfigure wireshark-common
```

wiresharkグループに自身のユーザーを追加する。

```bash
sudo usermod -a -G wireshark `whoami`
```

#### 2.7.5. SOME/IPパケットキャプチャの有効化
